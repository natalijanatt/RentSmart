...
import type { ISolanaService, SolanaAgreement, SolanaInitResult, SolanaRentPaymentTxResult, SolanaSettlementResult } from './interface';
...
export class SolanaService implements ISolanaService {
  ...
  async buildPayRentTx(
    ...
    const feeFromTenant = Math.floor((rentLamports * PLATFORM_FEE_BPS) / 10_000);
    const feeFromLandlord = Math.floor((rentLamports * PLATFORM_FEE_BPS) / 10_000);
    const landlordAmount = rentLamports - feeFromLandlord;
    const platformFeeTotal = feeFromTenant + feeFromLandlord;

    const tx = await this.program.methods
      .payRent(new BN(rentLamports))
      .accounts({
        agreement: pda,
        tenant: new PublicKey(tenantPubkey),
        landlord: new PublicKey(landlordPubkey),
