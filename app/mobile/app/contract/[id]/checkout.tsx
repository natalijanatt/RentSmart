...
import { Room, InspectionImage } from '@rentsmart/contracts';
...
export default function CheckoutScreen() {
  ...
  const [checkinImages, setCheckinImages] = useState<Record<string, InspectionImage[]>>({});
  ...
  // Load checkin reference images
  useEffect(() => {
    contractsService.getInspectionImages(id, 'checkin').then((res) => {
      const grouped = res.images.reduce<Record<string, InspectionImage[]>>((acc, img) => {
        if (!acc[img.room_id]) acc[img.room_id] = [];
        acc[img.room_id].push(img);
        return acc;
      }, {});
      setCheckinImages(grouped);
    }).catch(() => {});
  }, [id]);
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        if (contract.status === 'active' || contract.status === 'checkout_rejected') {
          const res = await contractsService.startCheckout(id);
          setSelectedContract(res.contract);
        }
        setUiState('room_select');
      } catch (err: any) {
        Alert.alert('Error', err?.response?.data?.error ?? err?.message ?? 'Failed to start check-out');
  ...
  const handleDoneWithRoom = async () => {
    ...
    try {
      ...
      Alert.alert('Upload failed', err?.response?.data?.error ?? err?.message ?? 'Upload failed');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      await contractsService.completeCheckout(id);
      router.replace(`/contract/${id}`);
    } catch (err: any) {
      Alert.alert('Error', err?.response?.data?.error ?? err?.message ?? 'Failed to complete check-out');
      setUiState('room_select');
    }
  };
  ...
  if (uiState === 'permissions' || uiState === 'starting' || uiState === 'completing') {
    const message = uiState === 'completing' ? 'Completing check-out…' : 'Starting check-out…';
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.centered}>
  ...
  if (uiState === 'camera' && selectedRoom) {
    ...
    return (
      <View style={styles.cameraContainer} {...swipePan.panHandlers}>
        ...
        {refImages.length > 0 && overlayVisible && currentOverlay && (
          <Image
            source={{ uri: currentOverlay.image_url }}
        ...
        {captured.length > 0 && (
          <View style={styles.newStrip}>
            <Text style={styles.newLabel}>New photos ({captured.length})</Text>
            <FlatList
              data={captured}
              horizontal
